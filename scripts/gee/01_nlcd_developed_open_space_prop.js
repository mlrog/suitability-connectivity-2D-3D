// =============================================================
// NLCD 2021 Developed Open Space — Proportion at 100 m
// =============================================================
// Purpose: Calculates the proportion of NLCD class 21
//          (Developed, Open Space) within each 100 m pixel
...
//     NLCD2021_DevelopedOpenSpace_Prop_100m_LA_UTM11.tif
...
// NLCD class to analyze (21 = Developed, Open Space)
var NLCD_CLASS = 21;
...
// =============================================================
// DEVELOPED OPEN SPACE PROPORTION (NLCD CLASS 21)
// =============================================================

// Create a binary mask: 1 where NLCD == class 21, else 0
var devOpenBinary = nlcdReprojected.eq(NLCD_CLASS)
  .rename('dev_open')
  .clip(studyBoundary);

// Multiply binary mask by pixel area to get m² of class 21
// within each 30 m cell
var devOpenArea = devOpenBinary.multiply(pixelArea);

// Aggregate class 21 area to 100 m (numerator)
var devOpenArea100m = devOpenArea.reduceResolution({
  reducer: ee.Reducer.sum(),
  maxPixels: 4096
});

// Calculate proportion: class 21 area / total cell area
// Output range: 0 (none) to 1 (entirely developed open space)
var devOpenProp100m = devOpenArea100m
  .divide(totalArea100m)
  .rename('dev_open_prop_100m')
...
Map.addLayer(
  ...
  'Developed Open Space proportion (100 m)'
);
...
var rangeCheck = devOpenProp100m.reduceRegion({
  reducer: ee.Reducer.minMax(),
  geometry: studyBoundary.geometry(),
  scale: OUTPUT_SCALE,
  maxPixels: 1e13
});
print('QA — Developed Open Space proportion range (expect 0–1):', rangeCheck);
...
Map.addLayer(
  ...
  { min: 0, max: 1, palette: ['white', 'blue'] },
  'QA — NLCD Class 21 binary (30 m)'
);

Map.addLayer(
  devOpenProp100m,
  { min: 0, max: 1, palette: ['white', 'blue'] },
  'QA — Developed Open Space proportion (100 m)'
);
...
Export.image.toDrive({
  image: devOpenProp100m,
  description: 'NLCD2021_DevelopedOpenSpace_Prop_100m_LA_UTM11',
