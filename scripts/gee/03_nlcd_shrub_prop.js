// =============================================================
// NLCD 2021 Shrub/Scrub — Proportion at 100 m
// =============================================================
// Purpose: Calculates the proportion of NLCD class 52
//          (Shrub/Scrub) within each 100 m pixel
...
//     NLCD2021_Shrub_Prop_100m_LA_UTM11.tif
...
// NLCD class to analyze (52 = Shrub/Scrub)
var NLCD_CLASS = 52;
...
// =============================================================
// SHRUB PROPORTION (NLCD CLASS 52)
// =============================================================

// Create a binary mask: 1 where NLCD == class 52, else 0
var shrubBinary = nlcdReprojected.eq(NLCD_CLASS)
  .rename('shrub')
  .clip(studyBoundary);

// Multiply binary mask by pixel area to get m² of class 52
// within each 30 m cell
var shrubArea = shrubBinary.multiply(pixelArea);

// Aggregate class 52 area to 100 m (numerator)
var shrubArea100m = shrubArea.reduceResolution({
  reducer: ee.Reducer.sum(),
  maxPixels: 4096
});

// Calculate proportion: class 52 area / total cell area
// Output range: 0 (none) to 1 (entirely shrub/scrub)
var shrubProp100m = shrubArea100m
  .divide(totalArea100m)
  .rename('shrub_prop_100m')
...
var rangeCheck = shrubProp100m.reduceRegion({
  reducer: ee.Reducer.minMax(),
  geometry: studyBoundary.geometry(),
  scale: OUTPUT_SCALE,
  maxPixels: 1e13
});
print('QA — Shrub proportion range (expect 0–1):', rangeCheck);

// (B) Side-by-side visual: 30 m source vs 100 m output
Map.addLayer(
  nlcdReprojected.eq(NLCD_CLASS).clip(studyBoundary),
  { min: 0, max: 1, palette: ['white', 'green'] },
  'QA — NLCD Class 52 binary (30 m)'
);

Map.addLayer(
  shrubProp100m,
  { min: 0, max: 1, palette: ['white', 'darkgreen'] },
  'QA — Shrub proportion (100 m)'
);
...
Export.image.toDrive({
  image: shrubProp100m,
  description: 'NLCD2021_Shrub_Prop_100m_LA_UTM11',
